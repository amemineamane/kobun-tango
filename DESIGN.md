...
**PTOKEN のキーが 1 文字なのは、全 44 編 179 段落ぶん（計 10,201 トークン）を書くと
ファイルが大きくなるため。**
...
| `data/works.js` | 作品 30 件（うち入試の出典 16 件は `exam` つき。本文を収めたのは 18 件） | 新規（アプリ側の追加データ） |
| `data/relations.js` | 単語間リンク 57 本 | 新規 |
| `data/workWords.js` | 作品タグ 26 件 | 新規 |
| `data/passages.js` | 教材 44 編・原文 179 段落（定番 37・入試の出典 7）。覚える語 812 件 | 新規 |
| `data/tokens/<文章id>.js` | 教材の全文品詞分解（1 文章 = 1 ファイル。**44 / 44 編ぶんそろっている**・計 10,201 トークン） | 新規。仕様は `docs/tokens-guide.md` |
...
教材 44 編（179 段落）を全文品詞分解すると 10,000 トークンを超える。
...
**なぜ 1 文章 1 ファイルか。** 全 44 編ぶんは 1 ファイルに収めると万行規模になり、
