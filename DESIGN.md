...
**PTOKEN のキーが 1 文字なのは、全 58 編 248 段落ぶん（計 13,057 トークン）を書くと
ファイルが大きくなるため。**
...
| `data/works.js` | 作品 38 件（うち入試の出典 16 件は `exam` つき。本文を収めたのは 26 件） | 新規（アプリ側の追加データ） |
| `data/relations.js` | 単語間リンク 57 本 | 新規 |
| `data/workWords.js` | 作品タグ 26 件 | 新規 |
| `data/passages.js` | 教材 58 編・原文 248 段落（定番 51・入試の出典 7）。覚える語 1,095 件 | 新規 |
| `data/tokens/<文章id>.js` | 教材の全文品詞分解（1 文章 = 1 ファイル。**58 / 58 編ぶんそろっている**・計 13,057 トークン） | 新規。仕様は `docs/tokens-guide.md` |
| `data/grammar.js` | 古典文法 149 項目（助動詞 29・助詞 56・敬語 38・活用 13・識別 13） | 新規。**用例は持たない**（5.6 参照） |
...
教材 58 編（248 段落）を全文品詞分解すると 13,000 トークンを超える。
...
- アイコンは外部フォントを使わずインライン SVG の線画（ヘッダのナビと同じ流儀：
  24×24・`currentColor`・`stroke-width 1.8`）。**公式ロゴは使わず、似せもしない**
  （X は交差する 2 本、LINE は紙飛行機。理由は `docs/copyright-review.md` §7）
  （商標ロゴの厳密な再現はしない）。
...
**なぜ 1 文章 1 ファイルか。** 全 58 編ぶんは 1 ファイルに収めると万行規模になり、
