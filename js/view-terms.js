 ...
 *     app_installed / history_reset / video_load / outbound_click）と 1 対 1 で対応している。
  ...
  function analyticsPart() {
    ...
    if (cfg.ga4) {
      ...
      out.push(ul([
        ...
        '外部リンク（YouTube チャンネルなど）をクリックしたこと（リンク先と、どの画面から開いたか）',
        'トップページの動画の再生ボタンを押し、YouTube の動画を読み込んだこと',
  ...
  function privacySection() {
    ...
    return sec('privacy', 'プライバシーポリシー', [
      ...
      ul([
        ...
        ['トップページ末尾の動画は Google LLC が提供する ', el('b', { text: 'YouTube' }),
          ' の埋め込みです。動画のサムネイルは本アプリのサーバーに置いており、',
          el('b', { text: '再生ボタンを押したときにだけ' }),
          ' YouTube を読み込みます。ページを開いただけの時点では YouTube への通信は発生せず、',
          '読み込んだ時点から YouTube のプライバシーポリシーが適用されます。']
      ]),
